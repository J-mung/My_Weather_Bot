...
import { type WeatherResponseMap } from "@/entities/weather/api/weather-api.types";
import type { WeatherStrategy } from "@/entities/weather/model/weather-model.types";
import {
...
export const weatherStrategyRegistry: { [K in keyof WeatherResponseMap]: WeatherStrategy<K> } = {
  ULTRA_NOW: {
    buildParams: () => getUltraSrtNcstBaseDateTime(),
    fetch: fetchUltraNow,
  },
  ULTRA_FORECAST: {
    buildParams: () => getUltraSrtFcstBaseDateTime(),
    fetch: fetchUltraForecast,
  },
  SHORT_FORECAST: {
    buildParams: () => getVilageFcstBaseDateTime(),
    fetch: fetchShortForecast,
  },
  TODAY_TEMP_RANGE: {
    buildParams: () => getVilageFcstTodayTempRangeBaseDateTime(),
    fetch: fetchShortForecast,
  },
};
