...
- 시/군/구/동 단위 키워드로 지역 검색을 지원합니다.
- 검색 결과 선택 시 메인 화면으로 이동 후 해당 위치 날씨 조회합니다.
- 검색 화면에서 북마크 추가/삭제 기능을 이용할 수 있습니다.
...
- 다음의 기능을 지원합니다.
  - 북마크 추가/삭제
  - 별칭 수정
  - 북마크 카드 클릭 시 메인 화면 이동 및 상세 조회
  - 로컬 저장소(localStorage) 기반 데이터 유지 (서버 없이 기능 구현을 위해 적용)
