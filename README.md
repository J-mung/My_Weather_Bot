...
### GPS 기반 위치 정보에 대한 지역명 미출력

- geolocation API로 위도, 경도를 구해 기상청이 제공하는 데이터와 대조하면 위치 정보를 얻을 수 있을 거라 생각했습니다.
- 그러나 대조하기 위해서는 위도, 경도를 보정해야 하고 이 과정에서 정확도가 떨어져서 명확한 위치를 찾기 쉽지 않았습니다.
- 별도로 조사해본 결과 카카오맵, 네이버지도, 구글맵 등의 3rd 파티 라이브러리를 활용하면 지역명을 구할 수 있다는 걸 알았습니다.
- 시간이 부족한 관계로 기능 구현은 진행하지 못해 "현재 위치"와 같이 고정된 값을 사용합니다.

  ※ 북마크 클릭을 통한 조회의 경우 지역명과 별칭 모두 정상 출력하고 있습니다.

### 메인화면 UI 구현 미완료

- 메인화면을 구성하는 각종 날씨 정보를 출력함에 있어 UI가 빈약한 상태입니다.
- 제한된 시간 안에 기본 기능 구현을 우선 시 하여 지금의 상태로 마무리 하게 됐습니다.

### 모바일 뷰 미구현

- 메인화면 UI와 마찬가지로 제한된 시간 안에 기본 기능 구현을 우선 시 하여 구현하지 못 했습니다.
- 예상하는 구현 결과를 간략하게 설명 드리면 다음과 같습니다.
  - 모바일 환경의 UX(Thumb zone)를 고려하여 검색창은 아래로 위치
  - 북마크 long touch로 편집 진행
  - 북마크 등록 시, 메인화면에서 좌우 슬라이드로 북마크 정보 조회 가능 
