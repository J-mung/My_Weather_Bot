...
### 2026-05-24 Phase 9 안정화 1차 구현

- AirKorea 응답의 `response.header.resultCode`가 `00`이 아니면 성공 데이터로 처리하지 않고 query error로 보낸다.
- Worker cache는 HTTP 200이면서 JSON `resultCode === "00"`인 응답만 저장하도록 제한했다.
  - 권한 오류, 429/5xx, 공공데이터포털 오류 JSON은 `no-store`로 반환한다.
  - Worker 환경변수 누락 응답에도 CORS header를 적용한다.
- 대기질 React Query key를 `["air-quality", sidoName]`으로 정규화했다.
  - 같은 시도 안에서 구/동만 바뀌는 경우 시도별 실시간 측정정보 요청을 중복 생성하지 않는다.
  - 측정소 선택은 캐시된 시도 데이터에서 클라이언트 측 mapper가 다시 계산한다.
- 행정구역 기반 측정소 매칭을 개선했다.
  - `충청북도 청주시 흥덕구 복대동` → `["복대동", "흥덕구", "흥덕", "청주시", "청주"]` 순서로 후보를 만든다.
  - 가장 구체적인 동/구/시군구 후보를 우선 비교한다.
  - 측정값이 있는 측정소를 우선 사용하고, 일치 후보가 없으면 측정값이 있는 대표 측정소로 fallback한다.
  - 선택 결과에는 `matchedKeyword`, `selectionReason`을 남겨 fallback 흐름을 추적할 수 있게 했다.
- 메인 화면 Fine Dust / Ultra Fine Dust 카드에 등급 badge를 추가했다.
  - `좋음`, `보통`, `나쁨`, `매우 나쁨`, `확인 중` 상태를 색상 tone으로 구분한다.
  - 로딩/실패/값 없음 상태의 설명 문구를 분리했다.
- AirQuality mapper 단위 테스트를 추가했다.
  - 시도명 변환
  - 세부 측정소 후보 생성
  - 구체 측정소 우선 선택
  - 대표 측정소 fallback
